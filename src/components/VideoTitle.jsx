...
const VideoTitle = ({ title, info,id }) => {
  ...
  return (
    <div className="w-screen aspect-video pt-[15%] px-6 md:px-12 absolute top-10 text-white  from-black ">
      <h1 className="text-base sm:text-3xl md:text-4xl xl:text-5xl font-bold">{title}</h1>
      <p className="  py-6 w-2/5 md:text truncate md:text-wrap">{info}</p>
      <div className=" flex">
       <div className="w-6/12 flex">
        <button className="bg-white hover:bg-opacity-80 text-black   rounded px-2 md:px-4 py-1 sm:py-2 text-sm sm:text-base md:text-lg lg:text-xl  hover:opacity-80 " >
      ...
      <button  className="w-9 h-9 rounded-full border-gray-500/50 hover: border-2 flex justify-center items-center ">{<VscMute className="font-bold text-lg"/>}</button>
      </div>
      </div>
    </div>
  );
